...
def validate_agent1_output(obj: Any) -> dict[str, Any]:
    ...
    required_str_fields = ["task_type", "notes"]
    ...
    for i, alt in enumerate(obj["alternatives"]):
        ...
        if (
            "why_rejected" not in alt
            or not isinstance(alt["why_rejected"], str)
            or not alt["why_rejected"].strip()
        ):
            raise ValueError(
                f"Поле `alternatives[{i}].why_rejected` обязательно и должно быть непустой строкой."
            )
...
def validate_agent2_output(obj: Any) -> dict[str, Any]:
    ...
    for i, step_obj in enumerate(obj["solution_steps"]):
        ...
        if "step" not in step_obj or not isinstance(step_obj["step"], int):
            raise ValueError(f"Поле `solution_steps[{i}].step` обязательно и должно быть int.")
        ...
        if "comment" not in step_obj or not isinstance(step_obj["comment"], str) or not step_obj["comment"].strip():
            raise ValueError(
                f"Поле `solution_steps[{i}].comment` обязательно и должно быть непустой строкой."
            )
...
def validate_agent3_output(obj: Any) -> dict[str, Any]:
    ...
    if not isinstance(summary, str) or not summary.strip():
        raise ValueError("Поле `alignment_assessment.summary` обязательно и должно быть непустой строкой.")
    ...
    for i, item in enumerate(reflections):
        ...
        if "step" not in item or not isinstance(item["step"], int):
            raise ValueError(f"Поле `step_reflection[{i}].step` обязательно и должно быть int.")
        for field in ("what_was_done", "issue", "improvement"):
            if field not in item or not isinstance(item[field], str) or not item[field].strip():
                raise ValueError(
                    f"Поле `step_reflection[{i}].{field}` обязательно и должно быть непустой строкой."
                )
